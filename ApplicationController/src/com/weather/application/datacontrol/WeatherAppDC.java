...
public class WeatherAppDC {
    ...
    public void invokeGetWeatherAPI(String countryName, String cityName) {
        ...
        public void run(){ 
          try {
                  ...
                  request.setRequestURI("/mobile/custom/WeatherAPI/getweather?country="+countryName+"&city="+cityName);            
    ...
    private void setCities(String citiesForCountry) {
        ...
        if(getCitiesResponse != null) {
            try {
                ...
                logger.logFine(getCitiesResponse.getBody().getGetCitiesByCountryResponse().getGetCitiesByCountryResult(), this.getClass().getSimpleName(), "josnToClassFailure");
                InputStream is = new ByteArrayInputStream(getCitiesResponse.getBody().getGetCitiesByCountryResponse().getGetCitiesByCountryResult().getBytes());
    ...
    private void setWeather(String weatherResponse) {
        ...
        try {
            currentWeatherResponse = (CurrentWeatherResponse) jbsh.fromJSON(CurrentWeatherResponse.class, weatherResponse);
        } catch (Exception e) {
            logger.logError(e.getMessage(), this.getClass().getSimpleName(), "josnToClassFailure");
            e.printStackTrace();
        }
        ...
        if(currentWeatherResponse != null) {
            try {
                ...
                String currentWeatherXML = currentWeatherResponse.getBody().getGetWeatherResponse().getGetWeatherResult();
                currentWeatherXML = currentWeatherXML.substring(currentWeatherXML.indexOf("<CurrentWeather>"), currentWeatherXML.length());
    ...
    public MBE getMobileBackend() {
        ...
        if(mobileBackend == null) {
            ...
            */
            ...
            MBEConfiguration mbeConfiguration = new MBEConfiguration("MCSRESTCONN", mobileBackendId, mbeAnonymousKey, appKeyAndroid,
